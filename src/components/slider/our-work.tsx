...
import OurWorkCard from "@/components/card/our-work";

import { Key, useEffect, useRef, useState } from "react";
import { Swiper, SwiperSlide } from "swiper/react";
import { Mousewheel } from "swiper/modules";
import { getGalleryImages } from "@/services/sanctuary";

import "swiper/css";

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export default function OurWorkSlider(onClick: any) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  if (gallery.length === 0) {
    return (
      <div className="flex justify-center items-center h-[70vh]">
        <p className="text-gray-400">Belum ada data gallery</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative">
      ...
      >
        {gallery.map(
          (data: { imageUrl: string }, index: Key | null | undefined) => (
            <SwiperSlide key={index}>
              <OurWorkCard imageUrl={data.imageUrl} onClick={onClick} />
            </SwiperSlide>
          )
        )}
      </Swiper>
